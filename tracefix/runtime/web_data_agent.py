...
def _call_model_json(
    *,
    ...
    Small local models occasionally emit almost-JSON. Retry once with an explicit
    correction instruction so a transient formatting mistake does not stop the
    runtime monitor before retrieval begins.
    """
    ...
    for attempt in range(2):
        if attempt:
            correction = (
                "Your previous response was not valid JSON. Return exactly one complete, valid JSON "
                "object with double-quoted keys and strings. Do not use Markdown or commentary."
            )
            if provider == "anthropic":
                payload["system"] = f"{system_prompt}\n\n{correction}"
                payload["temperature"] = 0
            else:
                payload["messages"][0]["content"] = f"{system_prompt}\n\n{correction}"
                payload["temperature"] = 0
        request = urllib.request.Request(
            endpoint,
        ...
        try:
            ...
            parse_error = exc
    raise ValueError(f"runtime agent model returned invalid JSON after 2 attempts: {parse_error}") from parse_error
