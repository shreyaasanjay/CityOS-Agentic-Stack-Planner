...
HANDLER_TEMPLATE_MARKER = "# tracefix-handler-template: smartroom-v8"
...
def _cached_handler_source(*, workspace: Path, kind: str, identity: str, plan: dict[str, Any], instructions: str, model: str, api_key: str) -> str:
    key_input = json.dumps({
        "template": "cityos-handler-v8-handler-protocol",
        "kind": kind,
        "identity": identity,
...
def _validate_generated_handler(source: str) -> str:
    source = source.strip()
    # Normalize the handler protocol for cached and newly generated handlers.
    # Older generated sources print `result` without the top-level `ok` field,
    # which the web-data harness correctly rejects as an unstructured failure.
    source = re.sub(
        r'(?m)^(\s*)print\(json\.dumps\(result\)\)\s*$',
        r'\1result.setdefault("ok", True)\n\1print(json.dumps(result))',
        source,
    )
