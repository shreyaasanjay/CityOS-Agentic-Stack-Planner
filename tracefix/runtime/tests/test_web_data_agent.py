...
def test_runtime_model_retries_once_after_malformed_json(monkeypatch):
    responses = iter([
        _Response({"choices": [{"message": {"content": '{"valid": true "violations": []}'}}]}),
        _Response({"choices": [{"message": {"content": '{"valid":true,"violations":[]}'}}]}),
    ])
    requests = []
    ...
    result, model = web_data_agent._call_model_json(
        provider="local",
        model="gemma3:4b",
        system_prompt="Return a monitor verdict.",
        user_payload={"event": "monitor_start"},
    )

    assert result == {"valid": True, "violations": []}
    assert model == "gemma3:4b"
    assert len(requests) == 2
    assert "previous response was not valid JSON" in requests[1]["messages"][0]["content"]
    assert requests[1]["temperature"] == 0


def test_runtime_model_reports_repeated_malformed_json(monkeypatch):
    ...
    with pytest.raises(ValueError, match="invalid JSON after 2 attempts"):
        web_data_agent._call_model_json(
            provider="local",
            model="gemma3:4b",
